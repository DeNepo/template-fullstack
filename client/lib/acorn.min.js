...
function isInAstralSet(e, t) {
  for (let a = 65536, n = 0; n < t.length; n += 2) {
  ...
  return e < 65
    ...
    : !91 <= e ||
        (e < 97
          ...
          : !123 <= e ||
            (e <= 65535
              ? e >= 170 && nonASCIIidentifierStart.test(String.fromCharCode(e))
        ...
        (e >= 65 &&
          (!91 <= e ||
            (e < 97
              ...
              : !123 <= e ||
                (e <= 65535
                  ? e >= 170 && nonASCIIidentifier.test(String.fromCharCode(e))
                  : !1 !== t &&
                    (isInAstralSet(e, astralIdentifierStartCodes) ||
                      isInAstralSet(e, astralIdentifierCodes))))));
}
const TokenType = function (e, t) {
  void 0 === t && (t = {}),
    this.label = e,
...
const beforeExpr = { beforeExpr: !0 };
const startsExpr = { startsExpr: !0 };
const keywords$1 = {};
/**
 * @param e
 * @param t
...
let warnedAboutEcmaVersion = !1;
/**
 * @param e
 */
function getOptions(e) {
  const t = {};
  for (const a in defaultOptions)
...
function pushComment(e, t) {
  return function (a, n, r, s, i, o) {
    const p = { type: a ? 'Block' : 'Line', value: n, start: r, end: s };
...
const Parser = function (e, t, a) {
  (this.options = e = getOptions(e)),
    this.sourceFile = e.sourceFile,
    this.keywords = wordsRegexp(
      keywords[
        6 <= e.ecmaVersion ? 6 : e.sourceType === 'module' ? '5module' : 5
      ],
    );
  let n = '';
  !0 !== e.allowReserved &&
    (n = reservedWords[e.ecmaVersion >= 6 ? 6 : e.ecmaVersion === 5 ? 5 : 3],
    e.sourceType === 'module' && (n += ' await')),
    this.reservedWords = wordsRegexp(n);
  let r = (n ? `${n} ` : '') + reservedWords.strict;
...
(Parser.prototype.parse = function () {
  ...
  (prototypeAccessors.canAwait.get = function () {
    for (var e, t = this.scopeStack.length - 1; t >= 0; t--) {
      if (e = this.scopeStack[t], e.inClassFieldInit) return !1;
      if (e.flags & SCOPE_FUNCTION) return (e.flags & SCOPE_ASYNC) > 0;
    }
    return (
  ...
  (prototypeAccessors.allowSuper.get = function () {
    const e = this.currentThisScope();
    let t = e.flags;
    var a = e.inClassFieldInit;
...
(pp.strictDirective = function (e) {
  for (;;) {
    ...
    const t = literal.exec(this.input.slice(e));
    if (!t) return !1;
    if ((t[1] || t[2]) === 'use strict') {
      skipWhiteSpace.lastIndex = e + t[0].length;
      const a = skipWhiteSpace.exec(this.input);
      var n = a.index + a[0].length;
      var r = this.input.charAt(n);
      return (
        r === ';' ||
        r === '}' ||
        (lineBreak.test(a[0]) &&
  ...
  (pp.unexpected = function (e) {
    this.raise(e == null ? this.start : e, 'Unexpected token');
  });
/**
 *
 */
...
(pp.checkPatternErrors = function (e, t) {
  if (e) {
    ...
    const a = t ? e.parenthesizedAssign : e.parenthesizedBind;
    a > -1 && this.raiseRecoverable(a, 'Parenthesized pattern');
  }
}),
  (pp.checkExpressionErrors = function (e, t) {
    if (!e) return !1;
    const a = e.shorthandAssign;
    let n = e.doubleProto;
    return t
      ? void (a >= 0 &&
          this.raise(
...
pp$1.parseTopLevel = function (e) {
  ...
  for (e.body || (e.body = []); this.type !== types.eof; ) {
    const a = this.parseStatement(null, !0, t);
    e.body.push(a);
  }
  if (this.inModule)
    for (
...
(pp$1.isLet = function (e) {
  ...
  const t = skipWhiteSpace.exec(this.input);
  var a = this.pos + t[0].length;
  let n = this.input.charCodeAt(a);
  if (n === 91 || n === 92 || (n > 55295 && n < 56320)) return !0;
  if (e) return !1;
  if (n === 123) return !0;
  if (isIdentifierStart(n, !0)) {
    for (var r = a + 1; isIdentifierChar((n = this.input.charCodeAt(r)), !0); )
      ++r;
    if (n === 92 || (n > 55295 && n < 56320)) return !0;
    const s = this.input.slice(a, r);
  ...
  (pp$1.isAsyncFunction = function () {
    ...
    let a = this.pos + t[0].length;
    return (
      !lineBreak.test(this.input.slice(this.pos, a)) &&
      this.input.slice(a, a + 8) === 'function' &&
  ...
  (pp$1.parseStatement = function (e, t, a) {
    ...
    switch ((this.isLet(e) && ((r = types._var), (n = 'let')), r)) {
      ...
      case types._export:
      case types._import:
        if (this.options.ecmaVersion > 10 && r === types._import) {
          skipWhiteSpace.lastIndex = this.pos;
          const i = skipWhiteSpace.exec(this.input);
          var o = this.pos + i[0].length;
  ...
  (pp$1.parseBreakContinueStatement = function (e, t) {
    ...
    for (var n, r = 0; r < this.labels.length; ++r)
      if (((n = this.labels[r]), null == e.label || n.name === e.label.name)) {
        if (n.kind != null && (a || n.kind === 'loop')) break;
        if (e.label && a) break;
      }
    return (
  ...
  (pp$1.parseForStatement = function (e) {
    ...
    let i = this.parseExpression(t <= -1 || 'await', s);
...
(pp$1.parseTryStatement = function (e) {
  if (
    ...
    if ((this.next(), this.eat(types.parenL))) {
      ...
      const a = t.param.type === 'Identifier';
  ...
  (pp$1.parseLabeledStatement = function (e, t, a, n) {
    for (var r, s = 0, o = this.labels; s < o.length; s += 1)
      (r = o[s]),
        r.name === t && this.raise(a.start, `Label '${t}' is already declared`);
    for (
      var p,
        c = this.type.isLoop
          ? 'loop'
      ...
      l >= 0 && ((p = this.labels[l]), p.statementStart === e.start);
      l--
    )
  ...
  (pp$1.parseVarId = function (e, t) {
    (e.id = this.parseBindingAtom()),
      this.checkLValPattern(e.id, t === 'var' ? BIND_VAR : BIND_LEXICAL, !1);
  });
...
(pp$1.parseFunction = function (e, t, a, n) {
  ...
  const r = this.yieldPos;
  var s = this.awaitPos;
  var i = this.awaitIdentPos;
  return (
    (this.yieldPos = 0),
    (this.awaitPos = 0),
  ...
  (pp$1.parseClass = function (e, t) {
    ...
    const n = this.enterClassBody();
    var r = this.startNode();
    let s = !1;
    for (r.body = [], this.expect(types.braceL); this.type !== types.braceR; ) {
      const i = this.parseClassElement(e.superClass !== null);
      i &&
        (r.body.push(i),
        i.type === 'MethodDefinition' && i.kind === 'constructor'
          ? (s &&
              this.raise(i.start, 'Duplicate constructor in the same class'),
            (s = !0))
          : i.key.type === 'PrivateIdentifier' &&
  ...
  (pp$1.parseClassElement = function (e) {
    ...
    if (
      ...
      !n &&
        ...
        !this.canInsertSemicolon()
          ? s = !0
          : n = 'async'),
      !n && (t >= 9 || !s) && this.eat(types.star) && (r = !0),
      !n && !s && !r)
    ) {
      const o = this.value;
    ...
    if (
      ...
      t < 13 || this.type === types.parenL || i !== 'method' || r || s)
    ) {
      const p = !a.static && checkKeyName(a, 'constructor');
      p &&
        i !== 'method' &&
  ...
  (pp$1.parseClassMethod = function (e, t, a, n) {
    ...
    const s = e.value = this.parseMethod(t, a, n);
    return (
      e.kind === 'get' &&
        s.params.length !== 0 &&
  ...
  (pp$1.enterClassBody = function () {
    const e = { declared: Object.create(null), used: [] };
...
(pp$1.parseExport = function (e, t) {
  ...
  (pp$1.checkPatternExport = function (e, t) {
    const a = t.type;
    if (a === 'Identifier') this.checkExport(e, t.name, t.start);
  ...
  (pp$2.parseBindingList = function (e, t, a) {
    for (var n = [], r = !0; !this.eat(e); )
      if (
        ...
        const i = this.parseMaybeDefault(this.start, this.startLoc);
...
(pp$3.checkPropClash = function (e, t, a) {
  if (
    ...
    if (this.options.ecmaVersion >= 6)
      return void (
        ...
        (t.proto &&
          (a
            ? a.doubleProto < 0 && (a.doubleProto = r.start)
            : this.raiseRecoverable(
                r.start,
  ...
  (pp$3.parseExpression = function (e, t) {
    const a = this.start;
    var n = this.startLoc;
    let r = this.parseMaybeAssign(e, t);
    if (this.type === types.comma) {
      const s = this.startNodeAt(a, n);
  ...
  (pp$3.parseExprOps = function (e, t) {
    const a = this.start;
    var n = this.startLoc;
    let r = this.parseMaybeUnary(t, !1);
    return this.checkExpressionErrors(t)
      ? r
      : r.start === a && r.type === 'ArrowFunctionExpression'
      ? r
      : this.parseExprOp(r, a, n, -1, e);
  }),
  (pp$3.parseExprOp = function (e, t, a, n, r) {
    let s = this.type.binop;
    if (s != null && (!r || this.type !== types._in) && s > n) {
      const i = this.type === types.logicalOR || this.type === types.logicalAND;
      var o = this.type === types.coalesce;
      o && (s = types.logicalAND.binop);
      const p = this.value;
      this.next();
      const c = this.start;
      var l = this.startLoc;
      var d = this.parseExprOp(this.parseMaybeUnary(null, !1), c, l, s, r);
      let u = this.buildBinary(t, a, e, d, p, i || o);
      return (
    ...
    if (this.isContextual('await') && this.canAwait)
      ...
      (i.operator = this.value),
        ...
        o
          ...
          : i.operator === 'delete' && isPrivateFieldAccess(i.argument)
...
(pp$3.parseExprSubscripts = function (e) {
  ...
  if (
    ...
    return n;
  const r = this.parseSubscripts(n, t, a);
  return (
    e &&
  ...
  (pp$3.parseSubscripts = function (e, t, a, n) {
    for (
      ...
      if (
        ...
        r.optional && (i = !0),
        r === e || r.type === 'ArrowFunctionExpression'
      ) {
        if (i) {
          const o = this.startNodeAt(t, a);
  ...
  (pp$3.parseSubscript = function (e, t, a, n, r, s) {
    const i = this.options.ecmaVersion >= 11,
    ...
    if (
      ...
      const l = new DestructuringErrors();
      var d = this.yieldPos;
      var u = this.awaitPos;
      let m = this.awaitIdentPos;
      (this.yieldPos = 0), (this.awaitPos = 0), (this.awaitIdentPos = 0);
      const h = this.parseExprList(
        types.parenR,
        this.options.ecmaVersion >= 8,
        !1,
        l,
  ...
  (pp$3.parseExprAtom = function (e) {
    ...
    switch (this.type) {
      ...
      case types.parenL:
        var p = this.start;
        var c = this.parseParenAndDistinguishExpression(a);
        return (
          e &&
  ...
  (pp$3.parseParenAndDistinguishExpression = function (e) {
    ...
    var a = this.start;
    let n = this.startLoc;
    var r = this.options.ecmaVersion >= 8;
    if (this.options.ecmaVersion >= 6) {
      this.next();
      let s;
      var i = this.start;
...
(pp$3.parseNew = function () {
  ...
  return (
    ...
    (e.arguments = this.eat(types.parenL)
      ? this.parseExprList(types.parenR, this.options.ecmaVersion >= 8, !1)
      : empty$1),
  ...
  (pp$3.parseObj = function (e, t) {
    ...
    for (a.properties = [], this.next(); !this.eat(types.braceR); ) {
      if (n) n = !1;
        ...
        break;
      const s = this.parseProperty(e, t);
  ...
  (pp$3.parseProperty = function (e, t) {
    ...
    return (
      ...
      e || o || this.options.ecmaVersion < 8 || a || !this.isAsyncProp(i)
        ? (n = !1)
        : ((n = !0),
      ...
      e.key.type === 'Identifier'
        ? ((a || n) && this.unexpected(),
          ...
          t
            ...
            ? (i.shorthandAssign < 0 && (i.shorthandAssign = this.start),
              (e.value = this.parseMaybeDefault(r, s, this.copyNode(e.key))))
  ...
  (pp$3.parseFunctionBody = function (e, t, a) {
    const n = t && this.type !== types.braceL;
    var r = this.strict;
    var s = !1;
    if (n)
  ...
  (pp$3.checkUnreserved = function (e) {
    ...
    if (
      ...
      const r = this.strict ? this.reservedWordsStrict : this.reservedWords;
      r.test(n) &&
        (!this.inAsync &&
          n === 'await' &&
          this.raiseRecoverable(
...
(pp$4.raise = function (e, t) {
  const a = getLineInfo(this.input, e);
  t += ` (${a.line}:${a.column})`;
  ...
  (pp$5.declareName = function (e, t, a) {
    ...
    if (t === BIND_LEXICAL) {
      ...
      (n = this.treatFunctionsAsVar
        ...
        : o.lexical.indexOf(e) > -1 || o.var.indexOf(e) > -1,
        o.functions.push(e);
    } else
      for (var p, c = this.scopeStack.length - 1; c >= 0; --c) {
        if (
          p = this.scopeStack[c],
...
(pp$6.finishNode = function (e, t) {
  ...
  (pp$6.copyNode = function (e) {
    const t = new Node(this, e.start, this.startLoc);
    for (const a in e) t[a] = e[a];
    return t;
  });
const TokContext = function (e, t, a, n, r) {
  this.token = e,
    this.isExpr = !!t,
...
(pp$7.initialContext = function () {
  ...
  (pp$7.inGeneratorContext = function () {
    for (var e, t = this.context.length - 1; t >= 1; t--)
      if (e = this.context[t], e.token === 'function') return e.generator;
    return !1;
  }),
  ...
  (types.parenR.updateContext = types.braceR.updateContext =
    function () {
      if (this.context.length === 1) return void (this.exprAllowed = !0);
...
const RegExpValidationState = function (e) {
  this.parser = e,
    this.validFlags =
      'gim' +
...
(RegExpValidationState.prototype.reset = function (e, t, a) {
  ...
  (RegExpValidationState.prototype.at = function (e, t) {
    ...
    if (!(t || this.switchU) || r <= 55295 || r >= 57344 || e + 1 >= n)
      return r;
    const s = a.charCodeAt(e + 1);
...
(pp$8.regexp_eatPatternCharacters = function (e) {
  for (
    var t = e.pos, a = 0;
    (a = e.current()) !== -1 && !isSyntaxCharacter(a);
...
pp$8.regexp_eatRegExpIdentifierPart = function (e) {
  const t = e.pos;
  const a = this.options.ecmaVersion >= 11;
  let n = e.current(a);
  return (e.advance(a),
  n === 92 &&
    this.regexp_eatRegExpUnicodeEscapeSequence(e, a) &&
...
function isRegExpIdentifierPart(e) {
  return (
    isIdentifierChar(e, !0) || e === 36 || e === 95 || e === 8204 || e === 8205
  );
}
(pp$8.regexp_eatAtomEscape = function (e) {
  ...
  (pp$8.regexp_eatZero = function (e) {
    return (
      !(e.current() !== 48 || isDecimalDigit(e.lookahead())) &&
...
(pp$8.regexp_eatLoneUnicodePropertyNameOrValue = function (e) {
  ...
  (pp$8.regexp_eatClassAtom = function (e) {
    ...
    if (e.eat(92)) {
      ...
      if (e.switchU) {
        const a = e.current();
        (a === 99 || isOctalDigit(a)) && e.raise('Invalid class escape'),
          e.raise('Invalid escape');
      }
      e.pos = t;
    }
    const n = e.current();
    return n !== 93 && ((e.lastIntValue = n), e.advance(), !0);
  }),
...
(pp$9.next = function (e) {
  ...
  (pp$9.readToken = function (e) {
    return isIdentifierStart(e, this.options.ecmaVersion >= 6) || e === 92
      ? this.readWord()
  ...
  (pp$9.skipBlockComment = function () {
    ...
    var t = this.pos;
    var a = this.input.indexOf('*/', (this.pos += 2));
    if (
    ...
    loop: while (this.pos < this.input.length) {
      ...
      switch (e) {
        ...
        default:
          if (
            (e > 8 && e < 14) ||
            (e >= 5760 && nonASCIIwhitespace.test(String.fromCharCode(e)))
          )
            ++this.pos;
          else break loop;
      }
  ...
  (pp$9.readToken_mult_modulo_exp = function (e) {
    ...
    var a = 1;
    let n = e === 42 ? types.star : types.modulo;
    return (
      this.options.ecmaVersion >= 7 &&
        e === 42 &&
        t === 42 &&
        (++a, (n = types.starstar), (t = this.input.charCodeAt(this.pos + 2))),
      t === 61 ? this.finishOp(types.assign, a + 1) : this.finishOp(n, a)
    );
  }),
  (pp$9.readToken_pipe_amp = function (e) {
    ...
    if (t === e) {
      if (this.options.ecmaVersion >= 12) {
        const a = this.input.charCodeAt(this.pos + 2);
        if (a === 61) return this.finishOp(types.assign, 3);
      }
      return this.finishOp(e === 124 ? types.logicalOR : types.logicalAND, 2);
    }
    return t === 61
      ? this.finishOp(types.assign, 2)
      : this.finishOp(e === 124 ? types.bitwiseOR : types.bitwiseAND, 1);
  }),
  ...
  (pp$9.readToken_lt_gt = function (e) {
    ...
    return t === e
      ...
      ? (t === 61 && (a = 2), this.finishOp(types.relational, a))
  ...
  (pp$9.readToken_eq_excl = function (e) {
    ...
    return t === 61
      ...
      : this.finishOp(e === 61 ? types.eq : types.prefix, 1);
  }),
  (pp$9.readToken_question = function () {
    ...
    if (e >= 11) {
      ...
      if (t === 46) {
        const a = this.input.charCodeAt(this.pos + 2);
        if (a < 48 || a > 57) return this.finishOp(types.questionDot, 2);
      }
      if (t === 63) {
        if (e >= 12) {
          const n = this.input.charCodeAt(this.pos + 2);
          if (n === 61) return this.finishOp(types.assign, 3);
        }
  ...
  (pp$9.finishOp = function (e, t) {
    const a = this.input.slice(this.pos, this.pos + t);
  ...
  (pp$9.readRegexp = function () {
    ...
    const r = this.input.slice(a, this.pos);
    ++this.pos;
    const s = this.pos;
    let i = this.readWord1();
  ...
  (pp$9.readInt = function (t, a, n) {
    for (
      var r = this.options.ecmaVersion >= 12 && void 0 === a,
        s = n && this.input.charCodeAt(this.pos) === 48,
        o = this.pos,
        p = 0,
        c = 0,
        l = 0,
    ...
    return (
      ...
      this.pos === o || (a != null && this.pos - o !== a) ? null : p
    );
  });
/**
 * @param e
...
(pp$9.readRadixNumber = function (e) {
  ...
  (pp$9.readNumber = function (e) {
    ...
    let a = this.pos - t >= 2 && this.input.charCodeAt(t) === 48;
    a && this.strict && this.raise(t, 'Invalid number');
    let n = this.input.charCodeAt(this.pos);
    if (!a && !e && this.options.ecmaVersion >= 11 && n === 110) {
      const r = stringToBigInt(this.input.slice(t, this.pos));
      return (
        ++this.pos,
        isIdentifierStart(this.fullCharCodeAtPos()) &&
          this.raise(this.pos, 'Identifier directly after number'),
    ...
    a && /[89]/.test(this.input.slice(t, this.pos)) && (a = !1),
      ...
      isIdentifierStart(this.fullCharCodeAtPos()) &&
        this.raise(this.pos, 'Identifier directly after number');
    const s = stringToNumber(this.input.slice(t, this.pos), a);
...
(pp$9.tryReadTemplateToken = function () {
  ...
  (pp$9.readTmplToken = function () {
    for (let e = '', t = this.pos; ; ) {
      ...
      const a = this.input.charCodeAt(this.pos);
  ...
  (pp$9.readWord1 = function () {
    this.containsEsc = !1;
    for (
      var e = '', t = !0, a = this.pos, n = this.options.ecmaVersion >= 6;
      this.pos < this.input.length;

    ) {
      const r = this.fullCharCodeAtPos();
